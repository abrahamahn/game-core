...
const fixture = JSON.parse(
  readFileSync(new URL('../../rust/fixtures/randomness-v1.json', import.meta.url), 'utf8'),
) as {
...
describe('cross-language randomness conformance', () => {
  it('pins raw output, bounded sampling, and shuffle ordering', () => {
    ...
    for (const vector of fixture.vectors) {
      ...
      expect(vector.outputs.map(() => raw.nextU64().toString()), vector.seed).toEqual(
        vector.outputs,
      );

      const bounded = new SeededRandom(seed);
      expect(vector.indexes.map(() => bounded.nextIndex(vector.indexUpperExclusive))).toEqual(
        vector.indexes,
      );
